...
const ArrayCreationTool = ({
   ...
   */
  const calculatePanelCount = useCallback(
   ...
   */
  const getAlignedDirection = useCallback(
    (startLatLng, endLatLng) => {
  ...
  const snapToAlignment = useCallback(
    (startLatLng, endLatLng, isHorizontal) => {
      const angleRad = (buildingRotation * Math.PI) / 180;
      const distance =
        window.google.maps.geometry.spherical.computeDistanceBetween(
          startLatLng,
    ...
    (event) => {
      ...
      if (
        ...
        if (!isInsideSetback) {
          ...
          alert(
            "⚠️ Please click inside the orange setback area (the safe zone away from the building edge)"
          );
          return;
        }
        ...
        originMarker.addListener("dragend", () => {
          ...
          console.log(
            "Origin marker dragged to:",
            originMarker.getPosition().toString()
          );
        });
        ...
        leftArrow.addListener("drag", (e) => {
          ...
          const spacingMeters =
            parseFloat(arrayManager.formData.distance_between_panels_ew) *
            0.3048;
          ...
          arrayManager.updateArrayRowsLeft(
            currentArrayRef.current,
            panelCount,
            mapRef.current
          );
        ...
        rightArrow.addListener("drag", (e) => {
          ...
          arrayManager.updateArrayRowsRight(
            currentArrayRef.current,
            panelCount,
            mapRef.current
          );
        ...
        upArrow.addListener("drag", (e) => {
          ...
          const spacingMeters =
            parseFloat(arrayManager.formData.distance_between_panels_ns) *
            0.3048;
          const unitLength = panelLengthMeters + spacingMeters;

          const panelCount = Math.max(
            1,
            Math.floor(finalDistance / unitLength)
          );
          arrayManager.updateArrayColsDown(
            currentArrayRef.current,
            panelCount,
            mapRef.current
          );
        ...
        downArrow.addListener("drag", (e) => {
          ...
          arrayManager.updateArrayColsUp(
            currentArrayRef.current,
            panelCount,
            mapRef.current
          );
  ...
  const handleMouseMove = useCallback(
    (event) => {
      ...
      if (creationStateRef.current === "placing") {
        ...
        const snappedEnd = snapToAlignment(
        ...
        const count = calculatePanelCount(
          dragStartPointRef.current,
          snappedEnd
        );
        setRowCount(count);
  ...
  const handleMouseUp = useCallback(() => {
    ...
    return;
  }, [isActive, previewLine, onArrayUpdated]);
