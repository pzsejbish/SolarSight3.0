...
function SolarSightComponent({ isGoogleMapsLoaded }) {
  ...
  useEffect(() => {
    console.log('SolarSight mounting with location state:', location.state);
  }, []);

  useEffect(() => {
    if (!location.state?.existingLayout?.panel_layout || !mapRef.current || !isGoogleMapsLoaded) {
      console.log('Skipping reconstruction - conditions not met');
      return;
    }
  
    const panels = location.state.existingLayout.panel_layout;
  ...
  }, [location.state?.existingLayout, mapRef.current, isGoogleMapsLoaded, formData]);
