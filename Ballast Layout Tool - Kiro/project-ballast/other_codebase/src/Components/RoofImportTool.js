...
const RoofImportTool = ({ 
  mapRef,
  onRoofImported,
  googleMapsApiKey 
}) => {
  const [isLoading, setIsLoading] = useState(false);
  const [address, setAddress] = useState('1478 Stone Point Drive, Roseville, CA 95661');
  ...
  const handleImportRoofCommercial = async () => {
    ...
    try {
      ...
      const buildingInsights = await findClosestBuilding(location, googleMapsApiKey);

      // Step 3: Convert to roof segments with proper transformation
      const roofSegments = convertRoofSegmentsToPolygons(buildingInsights, window.google);
      ...
      const buildingOutline = await createEdgeDetectedOutline(buildingInsights, googleMapsApiKey);
      ...
      const { value: importMode } = await Swal.fire({
        ...
        html: `
          <div style="text-align: left; margin: 20px 0;">
            ...
            <p>📏 Area: ${buildingOutline.metadata.buildingArea ? 
              (buildingOutline.metadata.buildingArea * 10.7639).toFixed(0) + ' ft²' : 
              'Unknown'}</p>
            <p>🏠 Roof Segments: ${roofSegments.length}</p>
            <p>📅 Imagery: ${buildingInsights.imageryDate?.month}/${buildingInsights.imageryDate?.year}</p>
      ...
      if (importMode === 'single') {
        ...
        const segmentList = roofSegments
          .map(seg => `<li style="text-align: left; margin: 5px 0;">${formatSegmentInfo(seg)}</li>`)
          .join('');
  ...
  const handleImportRoofResidential = async () => {
    ...
    try {
      ...
      console.log('🏢 Building found:', {
        ...
        distanceFromSearch: Math.sqrt(
          Math.pow((buildingInsights.center.latitude - location.lat()) * 111000, 2) +
          Math.pow((buildingInsights.center.longitude - location.lng()) * 111000 * Math.cos(location.lat() * Math.PI / 180), 2)
        ).toFixed(1) + 'm'
      });
      ...
      const result = await extractRoofSegmentsFromGeoTIFF(buildingInsights, googleMapsApiKey);
      ...
      if (roofSegments.length === 0) {
        throw new Error('No roof segments could be extracted from the GeoTIFF data');
      }
      ...
      Swal.fire({
        ...
        html: `
          <div style="text-align: left;">
            ...
            <p>📅 Imagery: ${buildingInsights.imageryDate?.month}/${buildingInsights.imageryDate?.year}</p>
            <br>
            <p>Imported ${roofSegments.length} roof segments traced from GeoTIFF:</p>
            <ul style="max-height: 200px; overflow-y: auto;">
              ${segmentList}
            </ul>
  ...
  return (
    <div style={{
      ...
      >
        {isLoading ? '⏳ Loading roof data...' : '🏢 Auto-Detect Roof (Commercial)'}
      </button>
      ...
      >
        {isLoading ? '⏳ Loading roof data...' : '🏠 Auto-Detect Roof (Residential)'}
      </button>
      ...
      <p style={{ 
        ...
        <strong>Commercial:</strong> Single polygon, aggressive simplification, perfect for large flat roofs.<br/>
        <strong>Residential:</strong> Traces actual roof geometry from GeoTIFF height data, preserves complex shapes.
      </p>
    </div>
  );
};
