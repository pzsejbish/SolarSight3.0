...
function SolarSightComponent({ formData, onSave, existingLayout }) {
  console.log('🏠 SolarSight component rendering');
  ...
  const [mapCenter, setMapCenter] = useState({ lat: 38.7521, lng: -121.2880 }); // Roseville, CA
  const [zoom, setZoom] = useState(20); // Close-in zoom for rooftop detail
  
  console.log('🗺️ SolarSight state:', { isGoogleMapsLoaded, mapCenter, zoom });
  ...
  useEffect(() => {
    console.log('SolarSight mounting with existingLayout:', existingLayout);
  }, [existingLayout]);

  useEffect(() => {
    if (!existingLayout?.panel_layout || !mapRef.current || !isGoogleMapsLoaded) {
    ...
    panels.forEach((polygonData, currentPolygonIndex) => {
      ...
      if (polygonData.building_width && polygonData.building_length) {
        ...
        console.log(`Using saved building dimensions for polygon ${currentPolygonIndex}:`, {
        ...
        if (polygonData.vertices && Array.isArray(polygonData.vertices)) {
          const dimensions = calculateBoundingBoxDimensions(polygonData.vertices);
          ...
          console.log(`Calculated building dimensions for polygon ${currentPolygonIndex}:`, {
            widthFeet: buildingWidthFeet,
            lengthFeet: buildingLengthFeet,
            areaFeet: buildingAreaFeet
          });
          ...
          console.warn(`No vertices available for polygon ${currentPolygonIndex} - using default dimensions`);
          buildingWidthFeet = 0;
      ...
      const path = polygonData.vertices.map(vertex => 
      ...
      const setbackDistanceMeters = parseFloat(formData.setback_distance) * 0.3048;
      ...
      console.log('Processing polygon layout:', {
        ...
        states: {
          ...
          obstructionCount: polygonData.layout.flat()
            .filter(x => !isNaN(parseFloat(x)) && x !== true && x !== false).length,
          intersectCount: polygonData.layout.flat().filter(x => x === 'intersects').length
        }
      });
      ...
      if (newPanelPolygons.panelPolygons) {
        ...
        newPanelPolygons.panelPolygons.forEach(panel => {
          ...
          console.log('Panel mapping:', {
            ...
            layoutRow,
            layoutCol,
            hasState: layoutRow !== undefined && layoutCol !== undefined && 
                      layoutRow < layout.length && layoutCol < layout[0].length
          });

          if (layoutRow !== undefined && layoutCol !== undefined && 
              layoutRow < layout.length && layoutCol < layout[0].length) {
            ...
            } else if (typeof state === 'number' || (typeof state === 'string' && !isNaN(state))) {
  ...
  }, [existingLayout, isGoogleMapsLoaded, formData]);
  ...
  const handleRectangleComplete = useCallback((rectangle) => {
    ...
    setPolygons(prevPolygons => {
      ...
      currentPolygon.panelPolygons.panelPolygons.forEach(panel => {
        if (doesPanelIntersectRectangle(panel, rectangle) && panel.isInsideSetback) {
          ...
          if (isEastWestSystem && panel.pairIndex !== undefined) {
            ...
            if (grid[row][col] !== panel) {
              console.warn("Grid panel reference mismatch. Updating grid reference.");
              grid[row][col] = panel;
            }
          }
        }
      ...
      console.log(`After rectangle: Selected panels: ${selectedCount}, Normal panels: ${normalCount}`);
...
const isPolygonSelfIntersecting = (pathArray) => {
  ...
  const handlePolygonComplete = useCallback((polygon) => {
    ...
    if (workflowState !== 'building') {
      console.log('🟢 IGNORING polygon - not in building mode, current state:', workflowState);
      console.log('🟢 This polygon should be handled by ObstructionDrawingTool');
      return;
    }
    ...
    const pathArray = path.map(latLng => ({ lat: latLng.lat(), lng: latLng.lng() }));
    ...
    if (boundingBoxDimensions.width < minSize || boundingBoxDimensions.length < minSize) {
      Swal.fire({
  ...
  const handleMapLoad = useCallback((map) => {
    console.log('🗺️ handleMapLoad called!');
    console.log('📍 Map object:', map);
    mapRef.current = map;
    setIsMapInitialized(true);
    setIsGoogleMapsLoaded(true); // Mark Google Maps as loaded
  ...
  }, [isMapInitialized, drawingManagerOptions, handlePolygonComplete, handleRectangleComplete, workflowState]);
  ...
  const addTextOverlay = useCallback((panel, height) => {
    ...
    textOverlay.onAdd = function() {
      ...
      div.style.textShadow = '1px 1px 2px rgba(255,255,255,0.7), -1px -1px 2px rgba(255,255,255,0.7), 1px -1px 2px rgba(255,255,255,0.7), -1px 1px 2px rgba(255,255,255,0.7)';
...
const handleArrayWorkflowNext = useCallback(() => {
  console.log('🔵 Array workflow Next clicked, current step:', arrayCreationStep);
  ...
  if (arrayCreationStep === 'origin' && currentArrayDraft) {
    ...
    console.log('🔵 Array finalized - latest from ArrayManager:', latestArray);
    ...
    setArrays(prev => {
      ...
      const existingArrayIndex = prev.findIndex(a => a.id === currentArrayDraft.id);
      ...
      if (existingArrayIndex !== -1) {
        ...
        console.log('✏️ Updating existing array at index:', existingArrayIndex);
        const updated = [...prev];
    ...
    if (arrayManager) {
      console.log('🔄 Syncing with ArrayManager, arrays:', arrayManager.getAllArrays());
    }
...
const handleArrayRotate = useCallback(() => {
  ...
  console.log('Array rotated from', currentRotation + '° to', newRotation + '° (rows and cols reset to 1)');
}, [currentArrayDraft, arrayManager, mapRef]);

const handleArrayWorkflowBack = useCallback(() => {
  console.log('🔴 Array workflow Back clicked, current step:', arrayCreationStep);
  ...
  if (arrayCreationStep === 'finalize' && currentArrayDraft) {
    ...
    if (currentArrayDraft.rightArrow) currentArrayDraft.rightArrow.setMap(null);
...
const handleEditArray = useCallback((arrayId) => {
  ...
  console.log('✏️ Hiding', array.panelPolygons.length, 'panels for editing');
  ...
  if (array.leftArrow && array.rightArrow && array.upArrow && array.downArrow) {
    ...
    const spacingEWMeters = parseFloat(arrayManager.formData.distance_between_panels_ew) * 0.3048;
    ...
    const spacingNSMeters = parseFloat(arrayManager.formData.distance_between_panels_ns) * 0.3048;
    ...
    const leftDistance = array.rowsLeft > 0 ? array.rowsLeft * unitWidth : minOffset;
    const rightDistance = array.rowsRight > 0 ? array.rowsRight * unitWidth : minOffset;
    const upDistance = array.colsUp > 0 ? array.colsUp * unitLength : minOffset;
    const downDistance = array.colsDown > 0 ? array.colsDown * unitLength : minOffset;
  ...
  console.log('✏️ Array has', array.panelPolygons.length, 'panels currently');
  setCurrentArrayDraft(array);
  ...
  console.log('✏️ Array loaded for editing - user can now navigate back through steps');
}, [arrayManager, mapRef]);
...
const handleToggleCreationMode = useCallback(() => {
  ...
  if (newMode) {
    console.log('🟢 Entering array creation mode - waiting for origin placement');
...
const toggleOrientation = useCallback(() => {
  ...
  setPolygons(prevPolygons => {
    ...
    if (
      ...
      console.error("Invalid panelPolygons structure:", selectedPolygon.panelPolygons);
      return prevPolygons;
    }
    ...
    selectedPolygon.totalRotationAngle = (selectedPolygon.rotationAngle + selectedPolygon.orientation) % 360;
    ...
    const pathArray = originalPath.map(latLng => ({ lat: latLng.lat(), lng: latLng.lng() }));
    ...
    selectedPolygon.panelPolygons.panelPolygons.forEach(panel => {
      const obstructed = obstructedPanels.find(p => p.row === panel.index.row && p.col === panel.index.col);
      if (obstructed) {
        panel.state = 'obstructed';
...
useEffect(() => {
  ...
  if (formData.sun_ballast_system === "east-west-system" && polygons.length > 0) {
    console.log("East-west system detected, applying 90 degree rotation to panels");
  ...
  useEffect(() => {
    if (polygonPath.length > 0 && setbackPolygon && mapRef.current) {
        ...
        const newPanelPolygons = processPolygon(polygonPath, setbackPolygon, mapRef, formData, isLandscape);
  ...
  useEffect(() => {
    ...
    if (drawingManagerRef.current) {
      if (isArrayCreationMode || workflowState === 'arrays') {
        ...
        console.log('🔷 Disabling drawing manager for building edit (polygon is editable)');
        drawingManagerRef.current.setDrawingMode(null);
        drawingManagerRef.current.setOptions({ drawingControl: false });
      } else if (workflowState === 'obstructions') {
        console.log('🔷 ========================================');
        console.log('🔷 WORKFLOW STATE IS OBSTRUCTIONS');
        console.log('🔷 Drawing manager will be controlled by ObstructionDrawingTool');
        console.log('🔷 ========================================');
        ...
        console.log('🔷 Disabling drawing manager for obstruction edit (polygons are editable)');
        drawingManagerRef.current.setDrawingMode(null);
        drawingManagerRef.current.setOptions({ drawingControl: false });
      }
    }
  ...
  useEffect(() => {
    if (selectedPolygonIndex !== null && polygons[selectedPolygonIndex]) {
      ...
      console.log('ArrayManager initialized for polygon', selectedPolygonIndex, 'with', polygonObstructions.length, 'obstructions');
    }
  }, [selectedPolygonIndex, polygons, formData, workflowState]);

  const shiftGrid = useCallback((direction, amount) => {
    if (selectedPolygonIndex === null) return;
    ...
    setPolygons(prevPolygons => {
      ...
      const shouldInvert = (selectedPolygon.isClockwise && !isTopEdge) || 
      ...
      if (isEastWestSystem) {
        ...
        selectedPolygon.panelPolygons.panelPolygons.forEach(panel => {
          if (panel.pairIndex !== undefined && pairStates.has(panel.pairIndex)) {
        ...
        selectedPolygon.panelPolygons.panelPolygons.forEach(panel => {
          const savedState = panelStates.find(p => p.row === panel.index.row && p.col === panel.index.col);
          if (savedState) {
            panel.state = savedState.state;
            if (savedState.state === 'obstructed') {
              panel.obstructionHeight = savedState.height;
...
const handleSaveLayout = useCallback(() => {    
  const layoutData = polygons.map((polygon, index) => {
    ...
    if (formData.sun_ballast_system === "east-west-system") {
      ...
      console.log(`Applied east-west transformation: rotation ${polygon.rotationAngle} → ${buildingRotation}`);
    }
   ...
   */
  const handleRoofImported = useCallback((importData) => {
    console.log('🌞 Roof imported from Solar API:', importData);
    console.log('🗺️ mapRef.current available?', !!mapRef.current);
    
    const { mode, polygon, segments, buildingInsights } = importData;
    ...
    if (mode === 'single') {
      ...
      const vertices = polygon.vertices.map(v => 
        new window.google.maps.LatLng(v.lat, v.lng)
      );
      ...
      const elevations = segments.map(s => parseFloat(s.metadata.heightMeters));
      ...
      segments.forEach((segment, index) => {
        console.log(`🔷 Creating segment ${index + 1}:`, segment.vertices.length, 'vertices');
        console.log(`   Bounds:`, segment.vertices);
        const vertices = segment.vertices.map(v => 
          new window.google.maps.LatLng(v.lat, v.lng)
        );
        
        // Color based on elevation (blue = low, red = high)
        const elevNormalized = maxElev > minElev ? 
          (parseFloat(segment.metadata.heightMeters) - minElev) / (maxElev - minElev) : 0.5;
      ...
      // TODO: In future, allow user to select which segments to use
      const firstSegment = segments[0];
      const vertices = firstSegment.vertices.map(v => 
        new window.google.maps.LatLng(v.lat, v.lng)
      );
  ...
  const handleWorkflowNext = useCallback(() => {
    ...
    if (workflowState === 'building-edit' && pendingBuildingPolygon) {
      ...
      console.log('Creating setback polygon with distance:', setbackDistanceFeet, 'feet');
      ...
      const updatedObstructions = obstructions.map(obstruction => {
        if (obstruction.polygon) {
          ...
          console.log('🔴 Updating obstruction path:', {
            ...
            pathChanged: JSON.stringify(obstruction.path) !== JSON.stringify(updatedPath)
          });
      ...
      const obstructionsWithSetbacks = generateObstructionSetbacks(updatedObstructions, 2); // 2ft default
  ...
  }, [workflowState, pendingBuildingPolygon, formData, mapRef, obstructions, selectedPolygonIndex, arrayManager]);
  ...
  }, [workflowState, pendingBuildingPolygon, drawingManagerRef, selectedPolygonIndex, polygons, obstructions]);
   ...
   */
  const handleFinishObstructions = useCallback((completedObstructions) => {
    console.log('Finishing obstructions:', completedObstructions);
    
    // Generate setback polygons for all obstructions
    const obstructionsWithSetbacks = generateObstructionSetbacks(completedObstructions, 2); // 2ft default
  ...
  console.log('🎯 About to render main UI, isGoogleMapsLoaded:', isGoogleMapsLoaded);

  // LoadScript in App.js ensures Google Maps is loaded before this component renders
  // So we don't need to check isGoogleMapsLoaded here
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', width: '100vw' }}>
      ...
      <div style={{ 
        ...
        {/* Roof Import Tool - Show in building workflow */}
        {useArrayMode && (workflowState === 'building' || workflowState === 'building-edit') && (
          <RoofImportTool
            mapRef={mapRef}
        ...
        {useArrayMode && (workflowState === 'building' || workflowState === 'building-edit' || workflowState === 'obstructions' || workflowState === 'obstructions-edit') && (
          <WorkflowControlPanel
            currentStep={workflowState}
            onNext={handleWorkflowNext}
            onBack={handleWorkflowBack}
            canGoNext={
              (workflowState === 'building-edit' && pendingBuildingPolygon !== null) ||
        ...
        {useArrayMode && workflowState === 'arrays' && (isArrayCreationMode || arrayCreationStep !== 'idle') && (
          <ArrayWorkflowPanel
            currentStep={arrayCreationStep === 'idle' ? 'origin' : arrayCreationStep}
        ...
        >
          {selectedPolygonIndex !== null && polygons[selectedPolygonIndex]?.isLandscape 
            ? 'Rotate Panels' 
            : 'Rotate Panels'}
        </button>
        ...
        {false && arrayCreationStep === 'rotate' && selectedPolygonIndex !== null && polygons[selectedPolygonIndex] && currentArrayDraft && (
          <div style={{ 
            width: '100%', 
            height: '200px', 
              ...
              <ErrorBoundary
                fallback={
                  <div style={{ 
                    ...
                    <div style={{ fontSize: '10px' }}>Check console for details</div>
                  </div>
                }
              >
                {(() => {
                  console.log('🎨 Rendering SolarPanelScene with:', {
                    totalRotationAngle: (polygons[selectedPolygonIndex]?.totalRotationAngle || 0) + (currentArrayDraft.rotation || 0),
                    tiltAngleDegrees: parseFloat(formData.tilt_angle) || 30,
                    isClockwise: polygons[selectedPolygonIndex]?.isClockwise ?? true
                  });
                  return (
                    <SolarPanelScene 
                      totalRotationAngle={
                        (polygons[selectedPolygonIndex]?.totalRotationAngle || 0) + (currentArrayDraft.rotation || 0)
                      }
                      tiltAngleDegrees={parseFloat(formData.tilt_angle) || 30}
                      isClockwise={polygons[selectedPolygonIndex]?.isClockwise ?? true}
                    />
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        {(() => {
          const shouldRender = useArrayMode && workflowState === 'obstructions' && selectedPolygonIndex !== null;
          console.log('🟣 Should render ObstructionDrawingTool?', shouldRender, {
            useArrayMode,
            workflowState,
            selectedPolygonIndex,
            hasPolygon: selectedPolygonIndex !== null && polygons[selectedPolygonIndex]
          });
          return shouldRender;
        })() && (
